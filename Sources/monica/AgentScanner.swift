...
final class AgentScanner: ObservableObject {
    ...
    private func listPanes() -> [RawPane] {
        let format = "#{pane_id}\t#{window_id}\t#{?session_group,#{session_group},#{session_name}}\t#{window_name}\t#{pane_current_path}\t#{pane_pid}"
    ...
    private func buildProcessTree() -> ProcessTree {
        ...
        for line in output.split(separator: "\n").dropFirst() {
            ...
            let name = (parts[2] as Substring).split(separator: "/").last.map(String.init) ?? String(parts[2])
